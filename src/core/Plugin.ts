...
export class Plugin {
  ...
  public static register(plugin: any) {
    ...
    if (plugin.name in this._registry) {
      const registered = this._registry[plugin.name]
      const pluginVersion = this.versionParse(plugin.version).number
      const registeredVersion = this.versionParse(registered.version).number

      if (pluginVersion > registeredVersion) {
        Common.warn(
          'Plugin.register:',
          this.toString(registered),
          'was upgraded to',
          this.toString(plugin)
        )
   ...
   */
  public static resolve(dependency: string) {
    return this._registry[this.dependencyParse(dependency).name]
  }
   ...
   */
  public static toString(plugin: IPlugin | string) {
   ...
   */
  public static isUsed<T extends { used: string[] }>(m: T, name: string) {
  ...
  public static isFor(
    ...
    const parsed = this.dependencyParse(plugin.for)
    return (
      !plugin.for ||
      (m.name === parsed.name && this.versionSatisfies(m.version, parsed.range))
    )
  }
  ...
  public static use<T extends IPlugin | string>(
    ...
    const dependencies = this.dependencies(m as Dependency)
    const sortedDependencies = Common.topologicalSort(dependencies).map(d => `${d}`)
    const status = []
    ...
    for (const d of sortedDependencies) {
      ...
      if ('used' in m && this.isUsed(m as { used: string[] }, plugin.name)) {
        continue
      }

      if (!this.isFor(plugin, m)) {
        Common.warn(
          'Plugin.use:',
          this.toString(plugin),
          'is for',
      ...
      if (plugin._warned) {
        status.push('🔶 ' + this.toString(plugin))
  ...
  public static dependencies(
    ...
    const parsedBase = this.dependencyParse(m)
    ...
    const plugin = typeof m === 'string' ? this.resolve(m) : (m as IPlugin)

    tracked[name] = (plugin.uses || []).map((dependency) => {
      if (this.isPlugin(dependency)) {
        this.register(dependency)
      }

      const parsed = this.dependencyParse(dependency)
      const resolved = this.resolve(dependency)

      if (resolved && !this.versionSatisfies(resolved.version, parsed.range)) {
        Common.warn(
          'Plugin.dependencies:',
          this.toString(resolved),
          'does not satisfy',
   ...
   */
  public static versionParse(range: string) {
    const pattern =
      /^(\*)|(\^|~|>=|>)?\s*((\d+)\.(\d+)\.(\d+))(-[0-9A-Za-z-+]+)?$/
   ...
   */
  public static versionSatisfies(version: string, range: string) {
    range = range || '*'

    const r = this.versionParse(range)
    const v = this.versionParse(version)
