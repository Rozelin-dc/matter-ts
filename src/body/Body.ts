...
/**
 * contains methods for creating and manipulating rigid bodies.
...
export class Body {
  ...
  public    label: string
    ...
    public  collisionFilter: ICollisionFilter
    public  slop: number
    public  timeScale: number
    public  render: IBodyRenderOptions
    public  events: Event | null
    public  bounds: Bounds
    public  chamfer: Object | null
    public  circleRadius: number
    public  positionPrev: Vector
    public  anglePrev
    public area: number
    public mass: number
    ...
    protected _original: boolean | null = null
   ...
   * @param options
   */
  constructor(options: IBodyDefinition) {
    ...
    this.type = options.type ?? 'body'
      ...
      this.vertices = options.vertices ? new Vertices(options.vertices, this) : new Vertices('L 0 0 L 40 0 L 40 40 L 0 40', this),
      this.position = options.position ?? new Vector(0, 0)
      this.force = options.force ?? new Vector(0, 0)
     ...
     this. speed = options.speed ?? 0
      ...
      this.motion = options.motion ?? 0
      this.sleepThreshold = options.sleepThreshold ?? 60
      this.density = options.density ?? 0.001
      this.restitution = options.restitution ?? 0
      this.friction = options.friction ??0.1
      this.frictionStatic = options.frictionStatic ?? 0.5
      ...
      this.bounds = options.bounds ?? new Bounds(this.vertices)
      this.chamfer = options.chamfer ?? null
      this.circleRadius = options.circleRadius ?? 0
      this.parent = options.parent ?? this
      this.axes = options.axes ?? null
      this.area = options.area ?? 0
      this.mass = options.mass ?? 0
      this.inertia = options.inertia ?? 0
      this.deltaTime = options.deltaTime ?? 1000 / 60

      this.positionPrev = cloneDeep(this.position)
   ...
   * @param options
   */
  protected _initProperties(options: IBodyDefinition) {
    ...
    this.vertices.rotate(this.angle, this.position)
    ...
    // render properties
    const defaultFillStyle = this.isStatic
        ? '#14151f'
        : Common.choose([
            '#f19648',
            '#f5d259',
    ...
    const  defaultStrokeStyle = this.isStatic ? '#555' : '#ccc'
    const  defaultLineWidth = this.isStatic && this.render.fillStyle ? 1 : 0
    this.render.fillStyle = this.render.fillStyle || defaultFillStyle
    this.render.strokeStyle = this.render.strokeStyle || defaultStrokeStyle
    this.render.lineWidth = this.render.lineWidth || defaultLineWidth
    if (this.render.sprite){
    this.render.sprite.xOffset +=
      -(this.bounds.min.x - this.position.x) /
      (this.bounds.max.x - this.bounds.min.x)
    this.render.sprite.yOffset +=
      -(this.bounds.min.y - this.position.y) /
   ...
   */
  public set<T extends keyof IBodyDefinition>(settings: T, value: IBodyDefinition[T])
  public set(settings: IBodyDefinition)
  public set<T extends keyof IBodyDefinition>(settings: T | IBodyDefinition, value?: IBodyDefinition[T]) {
    ...
    for (const property in settings) {
      const value = settings[property as keyof IBodyDefinition]
      switch (property) {
        case 'isStatic':
          this.setStatic(value as boolean)
          break
        case 'isSleeping':
